...
def resample_sine_waves(df1, df2, sampling='downsample'):
    """
    Resamples two sine waves to the same sampling rate.
    :param df1: DataFrame with columns "x" and "y" representing time and amplitude, respectively
    :param df2: DataFrame with columns "x" and "y" representing time and amplitude, respectively
    :param sampling: 'downsample', 'upsample', or 'average'
    :return: resampled_wave1, resampled_wave2 as DataFrames
    """

    # Check that the input is valid
    if df1.shape[1] < 2 or df2.shape[1] < 2:
        raise ValueError('Input DataFrames must have two columns')
    ...
    interpolator1 = interp1d(time1, df1['y'].values, fill_value="extrapolate")
    interpolator2 = interp1d(time2, df2['y'].values, fill_value="extrapolate")
    resampled_wave1 = interpolator1(t1_resampled)
    resampled_wave2 = interpolator2(t2_resampled)

    resampled_wave1 = pd.DataFrame({'x': t1_resampled, 'y': resampled_wave1})
    resampled_wave2 = pd.DataFrame({'x': t2_resampled, 'y': resampled_wave2})
...
def minimize_offset(df1, df2):
    """
    Minimizes the offset between two sine waves by shifting the second wave.
    :param df1: DataFrame with columns "x" and "y" representing time and amplitude, respectively
    :param df2: DataFrame with columns "x" and "y" representing time and amplitude, respectively
    :return: shifted_wave2 as a DataFrame
    """

    # Extract time and waveform from input DataFrames
    t1, waveform1 = df1['x'].values, df1['y'].values
    t2, waveform2 = df2['x'].values, df2['y'].values
    ...
    # Shift the second waveform by the calculated offset
    shifted_wave2 = pd.DataFrame({'x': t2 - offset, 'y': waveform2})
    print(f'{shifted_wave2["x"].iloc[0]=}')
    
    return df1, shifted_wave2